...
public class ElectrumService {
    ...
    public BigDecimal getBalance(String addressPrivateKey) {
        ECKey ecKey = DumpedPrivateKey.fromBase58(bitcoinNetworkParameters, addressPrivateKey).getKey();
        Address senderAddress = ecKey.toAddress(bitcoinNetworkParameters);
        Script script = ScriptBuilder.createOutputScript(senderAddress);
        String scriptHashReversed = HexUtils.toHexString(Sha256Hash.of(script.getProgram()).getReversedBytes());

        String responseJson = electrumRpcClient
                .sendCommand("blockchain.scripthash.get_balance", Collections.singletonList(scriptHashReversed));

        Balance balance;
        try {
            RpcResponse<Balance> response = objectMapper.readValue(responseJson, new TypeReference<RpcResponse<Balance>>() {
            });
            balance = response.getResult();
        } catch (IOException e) {
            throw new RuntimeException("Failed to parse get balance response: " + responseJson, e);
        }

        return balance.getConfirmed()
                .divide(new BigDecimal(SATOSHIS_PER_BTC), 10, BigDecimal.ROUND_HALF_UP);
    }
}
